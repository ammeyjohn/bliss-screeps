...
Structure.prototype.checkEnergy = function() {
  ...
  if (ratio < ENERGY_PERCENT ||
    ...
    if (storage != null) {
      const priority =  $.tasks[TASK_TRANSPORT].priority / ratio * this.getPriorityMultiplier(TASK_TRANSPORT);
      ...
      if (source != null) {
        const priority =  $.tasks[TASK_HARVEST].priority / ratio * this.getPriorityMultiplier(TASK_HARVEST);
...
Structure.prototype.repair = function() {
  ...
  if (ratio < HITS_PERCENT || this.data.hasTasks[TASK_REPAIR] && this.hits < this.hitsMax) {
    // 维修任务的优先级，血量越低优先级越高。
    const priority = $.tasks[TASK_REPAIR].priority / ratio * this.getPriorityMultiplier(TASK_REPAIR);
    let source = this.getCheapSource();
